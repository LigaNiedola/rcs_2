...
const filteredPhones2 = mobilePhones.filter((phone) => {
  const PhoneScreenSize = phone.specifications.screenSize;
  const PhoneNfc = phone.specifications.connectivity.nfc;

  return PhoneScreenSize <= filter2.screenSize && PhoneNfc === filter2.nfc;
});

console.log(filteredPhones2);
...
const filteredPhones3 = mobilePhones.filter((phone) => {
  const hasFeature = phone.specifications.camera.features.includes(filter3.feature);
  const filterResolution = filter3.resolution;
  const cameraResolution = phone.specifications.camera.resolution.match(/\d+(\.\d+)?/g);

  return cameraResolution >= filterResolution && hasFeature;
});

console.log(filteredPhones3);
...
const filter4 = {
  ...
  const filterColor = filter4.color;
  const filteredPhones4 = mobilePhones.filter((phone) => {
    const phoneColor = phone.specifications.colors;
    const containsColor = phoneColor.filter((color) => color.toLowerCase().includes(filterColor)).length;

    return containsColor;
  });
  
  console.log(filteredPhones4);
